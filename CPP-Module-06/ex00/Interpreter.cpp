...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/19 14:23:42 by gkehren           #+#    #+#             */
/*   Updated: 2022/11/22 11:53:51 by gkehren          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Interpreter::Interpreter(Interpreter const &other): str(other.str), type(other.type), ivalue(other.ivalue), fvalue(other.fvalue), dvalue(other.dvalue), cvalue(other.cvalue)
{
	for (int i = 0; i < 4; i++)
...
void Interpreter::parse(void)
{
	...
	for ( ; i < length; i++)
	{
		...
		else if (this->str[i] == 'e' && i < length - 1 && (this->str[i + 1] == '-'
				|| this->str[i + 1] == '+'
...
void Interpreter::fromFloat(void)
{
	...
	if (!this->floatIsValue() || this->fvalue > std::numeric_limits<int>::max() || this->fvalue < std::numeric_limits<int>::min())
		this->setFlag(TypeInt, this->isImpossible);
	return ;
}
...
void Interpreter::fromDouble(void)
{
	...
	if (!this->doubleIsValue() || this->dvalue > std::numeric_limits<int>::max() || this->dvalue < std::numeric_limits<int>::min())
		this->setFlag(TypeInt, this->isImpossible);
	return ;
}
